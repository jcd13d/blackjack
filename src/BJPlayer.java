...
public class BJPlayer extends Player implements Gambler {
    ...
    public void getPlayerMove(Deck deck) {
        ...
        if (!super.isDealer()) {
            for (int i = 0; i < hands.size(); i++) {
                ...
                input = util.getString(getMovePrompt());
    ...
    public void placeBet() {
      double betAttempt = Double.parseDouble(util.getString(getBetPrompt()));
      while (!checkBet(betAttempt)){
        betAttempt = Double.parseDouble(util.getString(getBetPrompt()));
        }
